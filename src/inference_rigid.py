...
def main(args):
    ...
    checkpoint_filename = '/apdcephfs/share_1364275/kaithgao/equidock_public/checkpts/oct20_Wdec_0.001#ITS_lw_10.0#Hdim_64#Nlay_5#shrdLay_T#ln_LN#lnX_0#Hnrm_0#NattH_50#skH_0.5#xConnI_0.0#LkySl_0.01#pokOTw_1.0#fine_F#/db5_model_best.pth'
    ...
    input_dir = './test_sets_pdb/gzq_input/'
    ground_truth_dir = './test_sets_pdb/gzq_gt/'
    output_dir = './test_sets_pdb/gzq_output/'
    ...
    for file in pdb_files:
        ...
        noise  = np.random.normal(2, 2, (809,3)).astype(dtype=np.float32)
        # noise = np.zeros((809,3)).astype(dtype=np.float32)
        unbound_ligand_all_atoms_pre_pos = ppdb_ligand.df['ATOM'][['x_coord', 'y_coord', 'z_coord']].to_numpy().squeeze().astype(np.float32)
        ppdb_ligand.df['ATOM'][['x_coord', 'y_coord', 'z_coord']] = unbound_ligand_all_atoms_pre_pos + noise
        unbound_ligand_all_atoms_pre_pos = ppdb_ligand.df['ATOM'][['x_coord', 'y_coord', 'z_coord']].to_numpy().squeeze().astype(np.float32)
        ...
        # noise_1  = np.random.normal(2, 0.1, bound_ligand_repres_nodes_loc_clean_array.shape).astype(dtype=np.float32)
        # noise_2  = np.random.normal(2, 0.1, bound_receptor_repres_nodes_loc_clean_array.shape).astype(dtype=np.float32)
        # bound_ligand_repres_nodes_loc_clean_array = bound_ligand_repres_nodes_loc_clean_array + noise_1
        # bound_receptor_repres_nodes_loc_clean_array = bound_receptor_repres_nodes_loc_clean_array + noise_2
