...
import dgl
from utils.protein_featurizers import residue_type_one_hot_dips_not_one_hot,  residue_type_one_hot_dips
...
def rigid_transform_Kabsch_3D(A, B, device):
    ...
    if torch.isnan(HA).any():
        HA = torch.rand(3,3).to(device)
...
def rigid_transform_Kabsch_3D_ori(A, B):
    assert A.shape[1] == B.shape[1]
    num_rows, num_cols = A.shape
    if num_rows != 3:
        raise Exception(f"matrix A is not 3xN, it is {num_rows}x{num_cols}")
...
def preprocess_unbound_bound(bound_ligand_residues, bound_receptor_residues, graph_nodes, pos_cutoff=8.0, inference=False):
   ...
   #########################
    ...
    # bound_predic_ligand_filtered = bound_ligand_residues
    ...
    # bound_predic_receptor_filtered = bound_receptor_residues
   ...
   ####################
    ...
    if not inference:
        ...
        n1 = list(range(0, bound_ligand_repres_nodes_loc_array.shape[0]))
        n2 = list(range(0, bound_receptor_repres_nodes_loc_array.shape[0]))
        all_index = torch.tensor(list(product(n1,n2)))
        nrows, ncols = np.array(all_index).shape
        dtype={'names':['f{}'.format(i) for i in range(ncols)],
            'formats':ncols * [np.array(all_index).dtype]}
        diff = np.array([x for x in set(tuple(x) for x in np.array(all_index)) - set(tuple(x) for x in np.array(positive_tuple).T)])
        # nagetive_ind = torch.tensor(diff).T[:, random.sample(range(np.shape(diff)[0]), 10*np.shape(np.array(positive_tuple))[1])]
        nagetive_ind = torch.tensor(diff).T
        positive_tuple = torch.tensor(positive_tuple)

        train_tuple = torch.cat((positive_tuple,nagetive_ind),dim=1)
        train_lable_tuple = torch.zeros((train_tuple.size(1)))
        train_lable_tuple[:positive_tuple.size(1)] = 1
        # train_lable_tuple[:,1] = 1 - train_lable_tuple[:,0]

        # train_tuple = torch.cat((positive_tuple,torch.tensor(diff.T)),dim=1)
        # train_lable_tuple = torch.zeros((train_tuple.size(1)))
        # train_lable_tuple[:positive_tuple.size(1)] = 1
        # # # train_lable_tuple[:,1] = 1 - train_lable_tuple[:,0]
        ...
        return unbound_predic_ligand_clean_list, unbound_predic_receptor_clean_list, \
               ...
               pocket_coors, positive_tuple, train_tuple, train_lable_tuple
...
def protein_to_graph_unbound_bound_residuesonly(unbound_ligand_predic,
    ...
    def l_or_r_align_unbound_and_bound(l_or_r_residue_representatives_loc_feat,
        ...
        ret_R_l_or_r, ret_t_l_or_r = rigid_transform_Kabsch_3D_ori(l_or_r_residue_representatives_loc_feat.T,
