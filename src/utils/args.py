...
parser.add_argument('-n_jobs', type=int, default=40, required=False, help="Number of worker for data preprocessing")


# Optim and Scheduler
parser.add_argument('-lr', type=float, default=3e-5, required=False)
parser.add_argument('-w_decay', type=float, default=1e-3, required=False)
parser.add_argument('-scheduler', default='ROP', choices=['ROP', 'warmup', 'cyclic'])
...
parser.add_argument('-bs', type=int, default=1, required=False)
parser.add_argument('-kab_dim', type=int, default=64, required=False)
parser.add_argument('-h_dim', type=int, default=16, required=False)
...
parser.add_argument('-graph_cutoff', type=float, default=22., required=False, help='Only for data caching and inference.')
...
parser.add_argument('-pocket_cutoff', type=float, default=11., required=False)
...
parser.add_argument('-atten_head', type=int, default=8, required=False)
parser.add_argument('-SEGCN_layer', type=int, default=1, required=False)


## Pocket OT:
parser.add_argument('-pocket_ot_loss_weight', type=float, default=1., required=False)
parser.add_argument('-gamma', type=float, default=0.2, required=False)
...
parser.add_argument('-p_rmsd', type=float, default=10)
parser.add_argument('-dropout', type=float, default=0., required=False)
parser.add_argument('-dp', type=float, default=0., required=False)
...
parser.add_argument('-prody', default=True, required=False)

parser.add_argument('-stage_2', default=False, required=False)
...
parser.add_argument('-toy', default=True, action='store_true') ## Train, validate and test only on DB5.5
...
def get_model_name(args):
    ...
    model_name = 'FLEXDOCK__'

    currentDateAndTime = datetime.datetime.now()
    model_name = model_name + str(currentDateAndTime.strftime("%H:%M:%S"))
    return model_name
########################################
...
if args['hyper_search']:
    ...
    while model_was_solved:
        ...
        model_was_solved = './stdouterr' + bbanner + ".txt"
...
args['checkpoint_dir'] = './checkpoints/' + banner
