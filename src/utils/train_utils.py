...
import dgl
from model.flexdock_model import *
...
def get_dataloader(args, log):
    ...
    train_loader = DataLoader(dataset=train_set,
                              batch_size=args['bs'],
                              shuffle=True,
                              collate_fn=partial(batchify_and_create_respective_graphs),
...
def batchify_and_create_respective_graphs(data):
    ...
    pocket_coors_ligand_list, pocket_coors_receptor_list, train_tuple_list, train_label_tuple_list = map(list, zip(*data))
    ligand_graph_list_pro, receptor_graph_list_pro = [], []
    for i, ligand_graph in enumerate(ligand_graph_list):
        receptor_graph = receptor_graph_list[i]
        ligand_graph_list_pro.append(ligand_graph)
        receptor_graph_list_pro.append(receptor_graph)

    batch_ligand_graph_list = dgl.batch(ligand_graph_list_pro)
    batch_receptor_graph_list = dgl.batch(receptor_graph_list_pro)

    return batch_ligand_graph_list, batch_receptor_graph_list, bound_ligand_repres_nodes_loc_array_list, bound_receptor_repres_nodes_loc_array_list,\
           pocket_coors_ligand_list, pocket_coors_receptor_list, train_tuple_list, train_label_tuple_list
...
def create_model(args, log):
    ...
    return FLEXDOCK_MODEL(args=args, log=log)
